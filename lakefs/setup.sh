#!/bin/sh
echo "⌛️ setting up the lakeFS Data+AI Summit 2022 Challenge..."
lakectl fs ls lakefs://dais-challenge/main/ >/dev/null 2>&1
if [ $? -eq 0 ]; then
    cat /data/banner.txt
    exit 0
fi
...
lakectl repo create lakefs://dais-challenge s3://dais-challenge-storage >/dev/null

# Upload data:
echo "📂 Downloading challenge datasets..."
export LAKECTL_BASE_URI="lakefs://dais-challenge/"
